...
function playRound(playerSelection, computerSelection) {
  if (playerSelection === computerSelection) {
    ...
    document.getElementById("scorePlayer").innerHTML =
      "Player:  " + playerScore;
    document.getElementById("scoreCpu").innerHTML =
  ...
  let logMessage = `Player => ${playerCurrentChoice} <br>Comp => ${computerCurrentChoice}<br> Score: ${playerScore} : ${computerScore}<br> ${currentLead(
    computerScore,
    playerScore
  )}`;
  ...
  if (rounds === 10) {
    ...
    if (playerScore > computerScore) {
      document.querySelector(
        ".roundWinner"
      ).innerHTML = `After 10 epic rounds, Player.<br> Score: ${playerScore} to ${computerScore}!`;
    } else if (playerScore < computerScore) {
      document.querySelector(
        ".roundWinner"
      ).innerHTML = `After 10 epic rounds, Computer.<br> Score: ${computerScore} to ${playerScore}!`;
    } else {
      document.querySelector(
        ".roundWinner"
      ).innerHTML = `After 10 epic rounds...It's a tie!<br> Score: ${playerScore} to ${computerScore}.`;
    }
    setTimeout(function () {
      document.getElementById("round").style.setProperty("display", "flex");
      document
        .querySelector(".scoreline")
